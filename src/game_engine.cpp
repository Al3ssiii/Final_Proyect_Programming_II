...
bool GameEngine::can_move_unit(int r, int c, int nr, int nc) {
    // Allows orthogonal movement 1 tile
    if (!((r == nr && std::abs(c - nc) == 1) || (c == nc && std::abs(r - nr) == 1)))
        return false;

    // Check if source has unit
    Tile &src = map.at(r, c);
    if (src.getUnit() == UnitType::NONE_U) return false;

    // Cannot move into Water (AG)
...
void GameEngine::player_move(int r, int c, int nr, int nc) {
    ...
    if (!map.in_bounds(r, c) || !map.in_bounds(nr, nc)) { std::cout << "Fuera de rango.\n"; return; }

    // Check ownership
    if (map.at(r, c).getUnitOwner() != Owner::J1) { std::cout << "Esa unidad no es tuya.\n"; return; }

    if (!can_move_unit(r, c, nr, nc)) { std::cout << "Movimiento invalido.\n"; return; }
...
void GameEngine::resolve_combat(int r, int c, int nr, int nc) {
    ...
    if (defStats.hp <= 0) {
        ...
        // Draw (usually attacker retreats or stays? Logic here implies both die or retreat)
        // For simplicity, if neither dies, nothing moves, but damage isn't persistent in this simple model
        // (Since units don't store current HP in Tile, only UnitType).
        // In this simple version, if neither dies, we bounce back.
...
void GameEngine::recruit(int r, int c, UnitType ut) {
    if (pa <= 0) { std::cout << "Sin PA.\n"; return; }
    if (!map.in_bounds(r, c)) { std::cout << "Fuera de rango.\n"; return; }
    Tile &t = map.at(r, c);
    if (t.getUnit() != UnitType::NONE_U) { std::cout << "Casilla ocupada.\n"; return; }

    bool allowed = false;
    // Can recruit near existing units or buildings of J1
    if (t.getOwner() == Owner::J1) allowed = true;
    if (t.getBuilding() == BuildingType::CU && t.getBuildingOwner() == Owner::J1) allowed = true;

    // Strict rule: Only on Barracks (CU)
...
void GameEngine::build(int r, int c, BuildingType bt) {
    ...
    if (t.getUnit() != UnitType::NONE_U) { std::cout << "Desocupa la casilla antes de construir.\n"; return; }
    if (t.getBuilding() != BuildingType::NONE_B) { std::cout << "Ya existe edificio.\n"; return; }


    int cost_metal = 1, cost_energy = 1;
    if (bt == BuildingType::TO) { cost_metal = 2; cost_energy = 1; }
...
void GameEngine::world_phase() {
    ...
    for (int r = 0; r < map.num_rows(); ++r) {
        for (int c = 0; c < map.num_cols(); ++c) { // Left to Right scan
            Tile &t = map.at(r, c);
            if (t.getUnitOwner() == Owner::J2 && t.getUnit() != UnitType::NONE_U) {
                int nr = r;
                int nc = c - 1;

                if (!map.in_bounds(nr, nc)) { nc = c; nr = r - 1; }
